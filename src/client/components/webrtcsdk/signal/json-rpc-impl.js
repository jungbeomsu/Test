...
class IonSFUJSONRPCSignal {
    ...
    async join(sid, uid, offer, joinConfig) {
        return this.call('join', {
            sid,
            uid,
            offer,
            config: { no_publish: false, no_subscribe: false, selective_pub_sub: false, ...joinConfig },
        });
    }
    trickle(trickle) {
